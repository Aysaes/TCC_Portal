...
export default function Dashboard({ auth, announcements }) {
    ...
    // The data is now nested inside announcements.data because of pagination
    ...
    // Helper to fix the messy Laravel labels (&laquo; Previous, etc)
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                        {announcementList.length === 0 ? (
                            ...
                            announcementList.map((item) => {
                                ...
                                return (
                                    <div key={item.id} className="relative flex flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-md transition-all hover:-translate-y-1 hover:shadow-lg">
                                        ...
                                        <div className="flex flex-1 flex-col p-5">
                                            ...
                                            <p className="mb-4 flex-1 whitespace-pre-wrap text-sm text-gray-600 leading-relaxed italic border-l-2 border-gray-100 pl-3">
                                                {item.content}
                                            </p>
                                        </div>
                                    </div>
                        ...
                        <div className="mt-12 flex justify-center">
                            <nav className="inline-flex items-center space-x-2 rounded-xl bg-white p-2 shadow-sm border border-gray-200">
                                {announcements.links.map((link, k) => {
                                    // Fix: Only use <Link> if url is NOT null. Otherwise use a <span>.
                                    return link.url ? (
                                        <Link
                                            key={k}
                                            href={link.url}
